<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<c:import url="/template/head_navbar.jsp"/>

<div style="padding-right: 65px;" id="slider">
    <ul class="subnav">
        <li style="background-color: grey;" ><a href="home.jsp"><i style="margin-right: 10px;" class="fa-solid fa-house"></i>    Home</a></li>
        <li><a href="majorload"><i style="margin-right: 10px;" class="fa-solid fa-landmark"></i>Majors</a></li>
        <li><a href="classload"><i style="margin-right: 10px;" class="fa-solid fa-building"></i>Classes</a></li>
        <li><a href="subjectload"><i style="margin-right: 10px;" class="fa-sharp fa-solid fa-book"></i>Subjects</a></li>
        <li><a href="#"><div style="display: inline-block;"><i style="margin-right: 10px;" class="fa-solid fa-graduation-cap"></i>Students Manager</div></a></li>
        <li><a href="teacherload"><div style="display: inline-block;"><i style="margin-right: 10px;" class="fa-solid fa-ruler"></i>Lecturers Manager</div></a></li>
    </ul>
   
    <div class="content">
        <div class="content_home">
            <h1 style="color: #00bcd4;">WellCome to SMS</h1>
            <p>
                SMS is a utility software to help manage students in a concise and complete way.
            </p>
            <img style="width: 60%; box-shadow: 0px 0px 10px 2px #888888; " src="image/1.png"/>
        </div>
        
    </div>   
</div>

<div id="footer">

</div>
</div>



</body>
</html>
